...
def _feature_cols(static: xr.Dataset | None, drivers: xr.Dataset | None = None) -> list[str]:
    static_cols = list(static.data_vars) if static is not None else []
    driver_cols = list(drivers.data_vars) if drivers is not None else []
    return LAG_SEASON_COLS + static_cols + driver_cols
...
def fit_predict_fold(
    ...
    fitted model.
    """
    cols = _feature_cols(static, drivers)

    layers = xr.Dataset() if static is None else static.copy()
    layers["in_space_train"] = spatial_train
    table = build_feature_table(ndvi, static=layers, drivers=drivers, dropna=False)
    ...
    # Rebuild the (time, y, x) cube and realign to the source grid: to_xarray
    # sorts the axes, which reorders real data's descending/float y coordinate.
    pred = table.set_index(["time", "y", "x"])["pred"].to_xarray().rename("gbt")
    return pred.reindex_like(ndvi), model
...
def walk_forward_gbt(
    ...
    for fi, fold in enumerate(folds):
        ...
        importances.append(pd.Series(model.feature_importances_, index=_feature_cols(static, drivers)))
